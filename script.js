class JarvisOverlay {
    constructor() {
        ...
        this.setupWindowsOverlayTracking(); // Send overlay bounds to main process for Windows
    ...
    setupVoiceRecording() {
        ...
        ipcRenderer.on('start-browser-recording', () => {
            this.startBrowserRecording();
        });
    ...
    async startBrowserRecording() {
        ...
        try {
            ...
            this.mediaRecorder.onstop = async () => {
                ...
                ipcRenderer.invoke('transcribe-audio-buffer', buffer);
            ...
            this.mediaRecorder.onerror = (event) => {
                ...
                ipcRenderer.send('voice-recording-error', 'Recording error: ' + event.error.message);
            };
            ...
            ipcRenderer.send('voice-recording-error', 'Failed to access microphone: ' + error.message);
        }
    }
    ...
    initializeElements() {
        ...
        if (typeof fetch !== 'undefined') {
            fetch('http://127.0.0.1:7242/ingest/91d30b1f-b1ff-43ce-a439-7eb1eb183847',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:1105',message:'Initialized menu elements',data:{settingsBtnExists:!!this.settingsBtn,settingsMenuExists:!!this.settingsMenu,settingsMenuId:this.settingsMenu?.id,settingsMenuClasses:this.settingsMenu?Array.from(this.settingsMenu.classList):null},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
        }
        // #endregion
    ...
    setupEventListeners() {
        ...
        // #region agent log
        if (typeof fetch !== 'undefined') {
            fetch('http://127.0.0.1:7242/ingest/91d30b1f-b1ff-43ce-a439-7eb1eb183847',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:1340',message:'Setting up settings button listener',data:{settingsBtnExists:!!this.settingsBtn,settingsMenuExists:!!this.settingsMenu,isElectron:this.isElectron},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
        }
        // #endregion
        if (this.settingsBtn) {
            // Make window interactive when hovering over settings button (for click-through mode)
            if (this.isElectron) {
                const { ipcRenderer } = require('electron');
                const isWindows = navigator.platform.toLowerCase().includes('win') || navigator.userAgent.toLowerCase().includes('windows');
                
                if (isWindows) {
                    // Force window to be interactive when mouse enters settings button area
                    this.settingsBtn.addEventListener('mouseenter', () => {
                        ipcRenderer.invoke('force-interactive').catch(() => {});
                    });
                    
                    // Also force interactive on pointer down (before click)
                    this.settingsBtn.addEventListener('pointerdown', () => {
                        ipcRenderer.invoke('force-interactive').catch(() => {});
                    });
                    
                    // Force interactive on mousedown as well
            ...
            this.settingsBtn.addEventListener('click', (e) => {
                // #region agent log
                fetch('http://127.0.0.1:7242/ingest/91d30b1f-b1ff-43ce-a439-7eb1eb183847',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:1361',message:'Hamburger button clicked',data:{settingsBtnExists:!!this.settingsBtn,settingsMenuExists:!!this.settingsMenu},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
                // #endregion
                e.stopPropagation();
                
                // Define isWindows and ipcRenderer at the start of the click handler so they're accessible throughout
                let isWindows = false;
                let ipcRenderer = null;
                if (this.isElectron) {
                    try {
                        ipcRenderer = require('electron').ipcRenderer;
                        isWindows = navigator.platform.toLowerCase().includes('win') || navigator.userAgent.toLowerCase().includes('windows');
                    } catch (e) {
                ...
                // #region agent log
                fetch('http://127.0.0.1:7242/ingest/91d30b1f-b1ff-43ce-a439-7eb1eb183847',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:1370',message:'About to check settingsMenu',data:{settingsMenuExists:!!this.settingsMenu},timestamp:Date.now(),sessionId:'debug-session',runId:'run2',hypothesisId:'G'})}).catch(()=>{});
                // #endregion
                if (this.settingsMenu) {
                    // #region agent log
                    fetch('http://127.0.0.1:7242/ingest/91d30b1f-b1ff-43ce-a439-7eb1eb183847',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:1372',message:'Settings menu element found',data:{hasHiddenClass:this.settingsMenu.classList.contains('hidden'),menuId:this.settingsMenu.id,menuClasses:Array.from(this.settingsMenu.classList)},timestamp:Date.now(),sessionId:'debug-session',runId:'run2',hypothesisId:'B'})}).catch(()=>{});
                    // #endregion
                    // #region agent log
                    const hasHidden = this.settingsMenu.classList.contains('hidden');
                    fetch('http://127.0.0.1:7242/ingest/91d30b1f-b1ff-43ce-a439-7eb1eb183847',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:1376',message:'Checking hidden class',data:{hasHiddenClass:hasHidden,hasHiddenValue:hasHidden,typeOfHasHidden:typeof hasHidden},timestamp:Date.now(),sessionId:'debug-session',runId:'run3',hypothesisId:'G'})}).catch(()=>{});
                    // #endregion
                    // #region agent log
                    fetch('http://127.0.0.1:7242/ingest/91d30b1f-b1ff-43ce-a439-7eb1eb183847',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:1388',message:'About to check if hasHidden',data:{hasHidden,willEnterIf:hasHidden===true},timestamp:Date.now(),sessionId:'debug-session',runId:'run4',hypothesisId:'H'})}).catch(()=>{});
                    // #endregion
                    try {
                        if (hasHidden) {
                            console.log('🔵 [DEBUG] INSIDE if block - hasHidden is true, about to show menu');
                            // Record the time when menu was opened FIRST to prevent blur from hiding it immediately
                            // This must be set BEFORE any operations that might trigger blur events
                            this._lastMenuOpenTime = Date.now();
                            // #region agent log
                            fetch('http://127.0.0.1:7242/ingest/91d30b1f-b1ff-43ce-a439-7eb1eb183847',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:1393',message:'INSIDE if block - Menu has hidden class, about to show',data:{isWindows,hasHidden,lastMenuOpenTime:this._lastMenuOpenTime},timestamp:Date.now(),sessionId:'debug-session',runId:'run4',hypothesisId:'C'})}).catch(()=>{});
                            // #endregion
                        // CRITICAL: Notify main process BEFORE showing menu to prevent click-through
                        if (isWindows) {
                            console.log('📋 [RENDERER] Sending menu-state-changed: true (BEFORE showing menu)');
                            // Send IMMEDIATELY before showing menu
                            ipcRenderer.send('menu-state-changed', true);
                            ipcRenderer.invoke('force-interactive').catch(() => {});
                        }
                        
                        // Now show the menu
                        this.settingsMenu.classList.remove('hidden');
                        const computedStyle = window.getComputedStyle(this.settingsMenu);
                        console.log('✅ [DEBUG] Menu shown - classes:', Array.from(this.settingsMenu.classList), 'display:', computedStyle.display, 'visibility:', computedStyle.visibility, 'opacity:', computedStyle.opacity, 'z-index:', computedStyle.zIndex);
                        // #region agent log
                        fetch('http://127.0.0.1:7242/ingest/91d30b1f-b1ff-43ce-a439-7eb1eb183847',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:1422',message:'Removed hidden class from menu',data:{hasHiddenClass:this.settingsMenu.classList.contains('hidden'),menuVisible:this.settingsMenu.offsetWidth>0&&this.settingsMenu.offsetHeight>0,menuDisplay:computedStyle.display,menuVisibility:computedStyle.visibility,menuOpacity:computedStyle.opacity,menuZIndex:computedStyle.zIndex},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'D'})}).catch(()=>{});
                        // #endregion
                        // Attach opacity slider listener when menu opens
                        this.attachOpacitySliderWhenReady();
                        
                        // Check menu visibility after a short delay
                        setTimeout(() => {
                            // #region agent log
                            fetch('http://127.0.0.1:7242/ingest/91d30b1f-b1ff-43ce-a439-7eb1eb183847',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:1380',message:'Menu visibility check after 10ms',data:{hasHiddenClass:this.settingsMenu.classList.contains('hidden'),menuVisible:this.settingsMenu.offsetWidth>0&&this.settingsMenu.offsetHeight>0,menuDisplay:window.getComputedStyle(this.settingsMenu).display,menuVisibility:window.getComputedStyle(this.settingsMenu).visibility},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
                            // #endregion
                        }, 10);
                        
                        // Send again after menu is shown to ensure it's received
                        if (isWindows) {
                            console.log('📋 [RENDERER] Menu shown, resending menu-state-changed: true');
                            // Send multiple times to ensure it's received
                            ipcRenderer.send('menu-state-changed', true);
                            ipcRenderer.invoke('force-interactive').then(() => {
                                console.log('✅ [RENDERER] force-interactive succeeded');
                            }).catch((e) => {
                                console.error('❌ [RENDERER] force-interactive failed:', e);
                            });
                            // Send multiple times to ensure it's received
                            setTimeout(() => {
                                console.log('📋 [RENDERER] Resending menu-state-changed: true (10ms)');
                                ipcRenderer.send('menu-state-changed', true);
                            }, 10);
                            setTimeout(() => {
                                console.log('📋 [RENDERER] Resending menu-state-changed: true (50ms)');
                                ipcRenderer.send('menu-state-changed', true);
                            }, 50);
                            setTimeout(() => {
                                console.log('📋 [RENDERER] Resending menu-state-changed: true (100ms)');
                                ipcRenderer.send('menu-state-changed', true);
                            }, 100);
                            // Update overlay bounds to include menu immediately and repeatedly
                            const updateBoundsWithMenu = () => {
                                ...
                                if (this.settingsMenu && !this.settingsMenu.classList.contains('hidden')) {
                                    ...
                                    // Add extra padding around menu to prevent click-through when approaching it
                                    // Increased padding to create a larger safe zone
                                    const menuPadding = 100;
                                    const minX = Math.min(bounds.x, menuRect.left - menuPadding);
                                    const minY = Math.min(bounds.y, menuRect.top - menuPadding);
                                    const maxX = Math.max(bounds.x + bounds.width, menuRect.left + menuRect.width + menuPadding);
                                    const maxY = Math.max(bounds.y + bounds.height, menuRect.top + menuRect.height + menuPadding);
                                    bounds = {
                                        x: minX,
                                        y: minY,
                                        width: maxX - minX,
                            ...
                            // Update immediately - no delay to prevent menu from disappearing
                            updateBoundsWithMenu();
                            // Update multiple times to ensure bounds are correct
                            setTimeout(updateBoundsWithMenu, 5);
                            setTimeout(updateBoundsWithMenu, 10);
                            setTimeout(updateBoundsWithMenu, 25);
                            setTimeout(updateBoundsWithMenu, 50);
                            setTimeout(updateBoundsWithMenu, 100);
                            // Keep updating while menu is open (in case it moves)
                            if (this.menuBoundsUpdateInterval) {
                                clearInterval(this.menuBoundsUpdateInterval);
                            }
                            this.menuBoundsUpdateInterval = setInterval(() => {
                                if (this.settingsMenu && !this.settingsMenu.classList.contains('hidden')) {
                                    updateBoundsWithMenu();
                                } else {
                                    clearInterval(this.menuBoundsUpdateInterval);
                                    this.menuBoundsUpdateInterval = null;
                                }
                            }, 100); // Update every 100ms instead of 200ms for faster response
                        }
                    } else {
                        console.log('🔴 [DEBUG] INSIDE else block - hasHidden is false');
                        // #region agent log
                        fetch('http://127.0.0.1:7242/ingest/91d30b1f-b1ff-43ce-a439-7eb1eb183847',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:1502',message:'Entered else block - menu does not have hidden class',data:{hasHiddenClass:this.settingsMenu.classList.contains('hidden')},timestamp:Date.now(),sessionId:'debug-session',runId:'run4',hypothesisId:'H'})}).catch(()=>{});
                        // #endregion
                        this.settingsMenu.classList.add('hidden');
                        // Notify main process that menu is closed
                        if (isWindows) {
                            console.log('📋 Sending menu-state-changed: false');
                            ipcRenderer.send('menu-state-changed', false);
                            // Stop bounds update interval
                            if (this.menuBoundsUpdateInterval) {
                                clearInterval(this.menuBoundsUpdateInterval);
                                this.menuBoundsUpdateInterval = null;
                            }
                            // Update overlay bounds to exclude menu
                            setTimeout(() => {
                                if (this.setupWindowsOverlayTracking) {
                        ...
                        console.error('❌ [DEBUG] ERROR in settings menu toggle:', error);
                        // #region agent log
                        fetch('http://127.0.0.1:7242/ingest/91d30b1f-b1ff-43ce-a439-7eb1eb183847',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:1538',message:'ERROR in if/else block',data:{error:error?.message,errorStack:error?.stack?.split('\n').slice(0,5).join('|')},timestamp:Date.now(),sessionId:'debug-session',runId:'run4',hypothesisId:'I'})}).catch(()=>{});
                        // #endregion
                    }
                }
                // Track for tutorial
        ...
        document.addEventListener('click', (e) => {
            ...
            // Don't close if menu is already hidden
            if (this.settingsMenu.classList.contains('hidden')) return;
            
            // Don't close if clicking on the settings button
            if (this.settingsBtn && (this.settingsBtn.contains(e.target) || e.target === this.settingsBtn)) {
                return;
            }
            
            // Don't close if clicking on the menu itself or any of its children
            if (this.settingsMenu.contains(e.target) || e.target === this.settingsMenu) {
                return;
            }
            ...
            if (this.isElectron) {
                try {
                    ...
                    const isWindows = navigator.platform.toLowerCase().includes('win') || navigator.userAgent.toLowerCase().includes('windows');
                    if (isWindows) {
                        console.log('📋 Sending menu-state-changed: false (click outside)');
                        ipcRenderer.send('menu-state-changed', false);
                        // Stop bounds update interval
                        if (this.menuBoundsUpdateInterval) {
                            clearInterval(this.menuBoundsUpdateInterval);
                    ...
                    console.error('Error closing menu on outside click:', error);
                }
            }
        });
        
        // Make settings menu interactive on Windows (for click-through mode)
        if (this.settingsMenu && this.isElectron) {
            const { ipcRenderer } = require('electron');
            const isWindows = navigator.platform.toLowerCase().includes('win') || navigator.userAgent.toLowerCase().includes('windows');
            
            // Track timeout for auto-closing menu when cursor leaves
            let menuAutoCloseTimeout = null;
            
            if (isWindows) {
                // Force window to be interactive when mouse enters menu area
                // This ensures menu stays visible and clickable even when mouse leaves main HUD
                this.settingsMenu.addEventListener('mouseenter', (e) => {
                    e.stopPropagation();
                    // Clear any pending auto-close timeout when mouse re-enters menu
                    if (menuAutoCloseTimeout) {
                        clearTimeout(menuAutoCloseTimeout);
                        menuAutoCloseTimeout = null;
                    }
                    ipcRenderer.invoke('force-interactive').catch(() => {});
                    // Keep menu visible - don't let it disappear
                    if (this.settingsMenu) {
                        this.settingsMenu.classList.remove('hidden');
                    }
                    // Immediately update bounds to include menu
                    if (this.overlay) {
                        const rect = this.overlay.getBoundingClientRect();
                        let bounds = {
                            x: rect.left,
                ...
                // Also keep menu visible on mouseover
                this.settingsMenu.addEventListener('mouseover', (e) => {
                    e.stopPropagation();
                    if (this.settingsMenu) {
                        this.settingsMenu.classList.remove('hidden');
                    }
                ...
                // Force interactive on pointer/mouse events over menu items
                this.settingsMenu.addEventListener('pointerdown', (e) => {
                    ipcRenderer.invoke('force-interactive').catch(() => {});
                });
                
                this.settingsMenu.addEventListener('mousedown', (e) => {
                    ipcRenderer.invoke('force-interactive').catch(() => {});
                });
                
                // Use event delegation to catch all interactions with menu items
                this.settingsMenu.addEventListener('mouseover', (e) => {
                    // Only trigger for actual menu items, not the menu container itself
                    if (e.target.closest('.settings-item, .model-item, .color-option')) {
                        ipcRenderer.invoke('force-interactive').catch(() => {});
                    }
                });
                
                // Force interactive on mouseenter for all menu items
                this.settingsMenu.addEventListener('mouseenter', (e) => {
                    if (e.target.closest('.settings-item, .model-item, .color-option')) {
                        ipcRenderer.invoke('force-interactive').catch(() => {});
                    }
                ...
                // Force interactive before any click on menu items
                this.settingsMenu.addEventListener('mousedown', (e) => {
                    if (e.target.closest('.settings-item, .model-item, .color-option')) {
                        e.stopPropagation();
                        ipcRenderer.invoke('force-interactive').catch(() => {});
                ...
                // Ensure clicks on menu items work
                ...
                this.settingsMenu.addEventListener('pointerenter', (e) => {
                    if (e.target.closest('.settings-item, .model-item, .color-option')) {
                        ipcRenderer.invoke('force-interactive').catch(() => {});
                    }
                });
                
                // Auto-close menu when cursor leaves for more than 2 seconds
                this.settingsMenu.addEventListener('mouseleave', (e) => {
                    e.stopPropagation();
                    ...
                    menuAutoCloseTimeout = setTimeout(() => {
                        if (this.settingsMenu && !this.settingsMenu.classList.contains('hidden')) {
                            // Close the menu
                            this.settingsMenu.classList.add('hidden');
                            // Notify main process that menu is closed
                            if (isWindows) {
                                console.log('📋 Auto-closing menu - cursor left for 2+ seconds');
                                ipcRenderer.send('menu-state-changed', false);
                                // Stop bounds update interval
                                if (this.menuBoundsUpdateInterval) {
                                    clearInterval(this.menuBoundsUpdateInterval);
        ...
        if (this.clearChatBtn) {
            // On Windows, force window to be interactive before clicking clear chat button
            if (this.isElectron) {
                const { ipcRenderer } = require('electron');
                const isWindows = navigator.platform.toLowerCase().includes('win') || navigator.userAgent.toLowerCase().includes('windows');
                
                if (isWindows) {
                    // Force interactive on hover and before click
                    this.clearChatBtn.addEventListener('mouseenter', () => {
                        ipcRenderer.invoke('force-interactive').catch(() => {});
                    });
                    this.clearChatBtn.addEventListener('mousedown', (e) => {
                        e.stopPropagation();
                        ipcRenderer.invoke('force-interactive').catch(() => {});
                    });
                    this.clearChatBtn.addEventListener('pointerdown', (e) => {
                        ipcRenderer.invoke('force-interactive').catch(() => {});
                    });
                }
            }
        ...
        if (this.accountInfoBtn) {
            // On Windows, force window to be interactive before clicking account button
            if (this.isElectron) {
                const { ipcRenderer } = require('electron');
                const isWindows = navigator.platform.toLowerCase().includes('win') || navigator.userAgent.toLowerCase().includes('windows');
                ...
                if (isWindows) {
                    // Force interactive on hover and before click
                    this.accountInfoBtn.addEventListener('mouseenter', () => {
                        ipcRenderer.invoke('force-interactive').catch(() => {});
                    });
                    ...
                    this.accountInfoBtn.addEventListener('pointerdown', (e) => {
                        ipcRenderer.invoke('force-interactive').catch(() => {});
                    });
                }
            }
            
            this.accountInfoBtn.addEventListener('click', (e) => {
                e.stopPropagation();
        ...
        // Menu only closes when clicking the hamburger button again (removed click-outside handler)
        // This allows the menu to stay open while interacting with it
        
        // Close menus and restore click-through when window loses focus (user clicked another window)
        window.addEventListener('blur', () => {
            // #region agent log
            if (typeof fetch !== 'undefined') {
                const menuHiddenBefore = this.settingsMenu?.classList.contains('hidden');
                fetch('http://127.0.0.1:7242/ingest/91d30b1f-b1ff-43ce-a439-7eb1eb183847',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:1909',message:'Window blur event fired',data:{settingsMenuHasHidden:menuHiddenBefore,menuIsOpen:!menuHiddenBefore},timestamp:Date.now(),sessionId:'debug-session',runId:'run7',hypothesisId:'E'})}).catch(()=>{});
            }
            // #endregion
            // CRITICAL: Don't hide menu if it's currently open
            // The menu should only close when the user clicks the hamburger button again, not on blur
            if (this.settingsMenu && !this.settingsMenu.classList.contains('hidden')) {
                console.log('🚫 [DEBUG] Blur event ignored - menu is currently open');
                return;
            }
            // Hide all menus (only if menu is already hidden, this is a no-op but keeps the code path consistent)
            this.hideSettingsMenu();
            // #region agent log
            if (typeof fetch !== 'undefined') {
                const menuHiddenAfter = this.settingsMenu?.classList.contains('hidden');
                fetch('http://127.0.0.1:7242/ingest/91d30b1f-b1ff-43ce-a439-7eb1eb183847',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:1886',message:'After hideSettingsMenu',data:{settingsMenuHasHidden:menuHiddenAfter},timestamp:Date.now(),sessionId:'debug-session',runId:'run2',hypothesisId:'E'})}).catch(()=>{});
            }
            // #endregion
            this.hideModelSubmenu();
        ...
        if (this.overlay) {
            ...
            // DISABLED: Don't close menu when mouse leaves overlay - menu should stay open until hamburger is clicked
            // Menu will only close when clicking the hamburger button again or clicking the Close button
            /*
            this.overlay.addEventListener('mouseleave', () => {
                if (this.isElectron && isCurrentlyInteractive) {
                    ...
                    menuCloseTimeout = setTimeout(() => {
                        // CRITICAL: Don't close menu if it's currently open
                        // The menu should only close when the user clicks the hamburger button again
                        if (this.settingsMenu && !this.settingsMenu.classList.contains('hidden')) {
                            return;
                        }
                        // Close any open menus when mouse leaves overlay (only if menu is already closed)
                        this.hideSettingsMenu();
                        this.hideModelSubmenu();
                        this.hideSettingsSubmenu();
                        this.hideColorSubmenu();
            ...
            */
            ...
            // DISABLED: Window is already interactive from main.js - no need to spam make-interactive
            // Disable cursor following behavior on Windows - it causes the overlay to follow cursor
            // this.overlay.addEventListener('mousemove', (e) => {
            //     if (this.isElectron && e.buttons > 0) {
            //         // Mouse is being held down, keep interactive
            //         // Only do this if we're actually dragging (not just moving cursor)
            //         clearTimeout(clickThroughTimeout);
            ...
            // Windows: Don't set click-through - window should stay interactive
            // The main process keeps the window interactive, so we don't need to toggle it here
            // if (this.isElectron) {
            //     const { ipcRenderer } = require('electron');
            //     setTimeout(() => {
            //         ipcRenderer.invoke('make-click-through').catch(() => {});
    ...
    showLimitExceededNotification() {
        ...
        if (this.isElectron && window.require) {
            ...
            // Window is already interactive from main.js - no need to spam
            // ipcRenderer.invoke('make-interactive');
            
            // Keep making it interactive on any mouse movement over the notification
            // DISABLED - window is already interactive, this causes spam
    ...
    setupDragFunctionality() {
        ...
        this.dragHandle.addEventListener('mousedown', (e) => {
            // Check if this is part of a double-click (within 300ms of last click)
            const currentTime = Date.now();
            if (currentTime - lastClickTime < 300) {
                isDoubleClicking = true;
                // Don't start dragging if it's a double-click
                return;
            }
        ...
        document.addEventListener('mousemove', (e) => {
            if (!isDragging || isDoubleClicking) return;
        ...
        this.dragHandle.addEventListener('dblclick', (e) => {
            ...
            // Force stop dragging immediately
            isDoubleClicking = true;
            isDragging = false;
            this.overlay.style.cursor = 'default';
            ...
            // Ensure drag state stays cleared
            setTimeout(() => {
                isDoubleClicking = false;
            }, 500);
        });
    ...
    // Windows: Send overlay bounds to main process for mouse tracking
    setupWindowsOverlayTracking() {
        if (!this.isElectron) return;
        ...
        // Function to send current overlay bounds (including menu if open)
        const sendOverlayBounds = () => {
            if (!this.overlay) return;
            ...
            ipcRenderer.invoke('get-window-bounds').then(windowBounds => {
                if (windowBounds) {
                    ...
                    // Fallback: assume window is at (0,0) since it's fullscreen
                    ipcRenderer.send('update-overlay-bounds', {
        ...
        // Send bounds periodically (overlay can be dragged)
        setInterval(sendOverlayBounds, 100);
        
        // Also send immediately
        setTimeout(sendOverlayBounds, 500);
    }
...
${currentQuestion}`;
    ...
    clearChatHistory() {
        ...
        // On Windows, ensure window stays interactive and focus text input after menu closes
        if (this.isElectron && window.require) {
            const { ipcRenderer } = window.require('electron');
            const isWindows = navigator.platform.toLowerCase().includes('win') || navigator.userAgent.toLowerCase().includes('windows');
            
            if (isWindows) {
                // Send menu-state-changed: false to properly close menu state tracking
                ipcRenderer.send('menu-state-changed', false);
                
                // Keep window interactive and focus text input after a short delay
                setTimeout(() => {
                    ipcRenderer.invoke('force-interactive').catch(() => {});
                    if (this.textInput) {
                        this.textInput.focus();
                ...
                // On other platforms, just focus the input
                if (this.textInput) {
                    this.textInput.focus();
                }
            }
        } else {
            // Not Electron, just focus input
            if (this.textInput) {
                this.textInput.focus();
            }
        }
...
User Question: ${question}`;
    ...
    async promptDocumentName(defaultName = '') {
        return new Promise((resolve) => {
            ...
            // Make window interactive when showing modal (force interactive, like menu)
            if (this.isElectron && window.require) {
                const { ipcRenderer } = window.require('electron');
                const isWindows = navigator.platform.toLowerCase().includes('win') || navigator.userAgent.toLowerCase().includes('windows');
                if (isWindows) {
            ...
            const cleanup = () => {
                ...
                // Notify main process that modal is closing
                if (this.isElectron && window.require) {
                    const { ipcRenderer } = window.require('electron');
                    const isWindows = navigator.platform.toLowerCase().includes('win') || navigator.userAgent.toLowerCase().includes('windows');
                    if (isWindows) {
            ...
            setTimeout(() => {
                if (!this.isDraggingOutput && !this.isResizing && this.isElectron) {
                    ...
                    if (mouseX < rect.left || mouseX > rect.right || mouseY < rect.top || mouseY > rect.bottom || mouseX === 0 && mouseY === 0) {
                        ...
                        // Mouse is still over overlay - window is already interactive, no need to spam
                        // ipcRenderer.invoke('make-interactive');
                    }
                }
            }, 50);
